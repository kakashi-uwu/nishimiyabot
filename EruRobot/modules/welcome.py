...
from EruRobot.modules.helper_funcs.chat_status import (
    is_user_ban_protected,
    user_admin,
)
from EruRobot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from EruRobot.modules.helper_funcs.msg_types import get_welcome_type
from EruRobot.modules.helper_funcs.handlers import MessageHandlerChecker
from EruRobot.modules.helper_funcs.string_handling import (
    escape_invalid_curly_brackets,
    markdown_parser,
)
from EruRobot.modules.log_channel import loggable
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                ...
                welcome_log = (
                    ...
                    f"#USER_JOINED\n"
                    f"my baby just joined the battle .....Alex play ... attention!"
                )
                continue
            ...
            if new_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "Attention morons my buddy just joined the battle!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in DEMONS:
                update.effective_message.reply_text(
                    "Here comes my aniki ..... switch off the phone!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            if new_mem.id in WOLVES:
                update.effective_message.reply_text(
                    "So who do we have here ? Oh he's the one in my simp list...one more simp ufff!!", reply_to_message_id=reply
                )
                continue
